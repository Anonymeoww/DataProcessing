...
def extract_movies(dom):
    ...
    for movie in dom.find_all("div", class_="lister-item-content"):
        actors = []
        titlename = movie.find("h3", class_="lister-item-header").a.text
        ...
        release = re.sub('[^0-9]', '', year.text)
        texts = movie.find_all("p", class_="")
        for block in texts:
            actorsraw = block.select("a[href*=st]")
            for actor in actorsraw:
                actor = actor.text
                actors.append(actor)
                actorslist = ",".join(str(x) for x in actors)
        time = movie.find("p", class_="text-muted")
        timer = time.find("span", class_="runtime").text
        runtime = re.sub('[^0-9]', '', timer)
        movie_info = {'title': titlename, 'rating': rating, 'release': release, 'actors': actorslist, 'runtime': runtime}
